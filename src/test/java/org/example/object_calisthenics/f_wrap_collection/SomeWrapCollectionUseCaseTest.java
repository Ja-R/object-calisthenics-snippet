...
class SomeWrapCollectionUseCaseTest {

	@Mock
	private VetRepository vetRepository;

	private SomeWrapCollectionUseCase useCase;

	@BeforeEach
	void setUp() {
		useCase = new SomeWrapCollectionUseCase(vetRepository);
	}

	private final Person person1 = new Person(
			new Name("Firstname1", "Surname1"),
			new Address("Street1", "City1"),
			Pets.of(List.of(new Pet("Rex", "Dog"), new Pet("Billy", "Cat")))
	);
	private final Person person2 = new Person(
			new Name("Firstname2", "Surname2"),
			new Address("Street2", "City2"),
			Pets.of(List.of(new Pet("Coucou", "Bird"), new Pet("Garou", "Cat"), new Pet("Mickey", "Mousse")))
	);
	private final Person person3 = new Person(
			new Name("Firstname3", "Surname3"),
			new Address("Street3", "City3"),
			Pets.of(List.of(new Pet("Flaky", "Snake")))
	);
	private final Vet vet = new Vet("Little", Clientele.of(List.of(person1, person2, person3)));
